\chapter{Thiết Kế, Chế Tạo Và Đánh Giá Hệ Thống}
...
\begin{small}
\begin{tabular}{lcllp{6.5cm}}
\toprule
\textbf{Tên Pin cấu hình} & \textbf{Chân GPIO} & \textbf{Kiểu kết nối} & \textbf{Thiết bị ngoại vi} & \textbf{Giao tiếp / Vai trò} \\
\midrule
\texttt{DHTPIN} & 14 & Digital Input & Cảm biến DHT22 & Giao thức 1-wire (Nhiệt/Ẩm) \\
\texttt{MQ135\_PIN} & 34 & Analog Input & Cảm biến MQ135 & ADC1\_CH6 (Đọc điện áp khí) \\
\texttt{GP2Y\_PIN} & 35 & Analog Input & Cảm biến bụi GP2Y & ADC1\_CH7 (Đọc cường độ bụi) \\
\texttt{GP2Y\_LED\_PIN} & 25 & Digital Output & Cảm biến bụi GP2Y & Điều khiển LED hồng ngoại (Active LOW) \\
\texttt{WARNING\_LED\_PIN} & 26 & Digital Output & LED màu đỏ & Phát tín hiệu cảnh báo tại chỗ \\
\texttt{BUZZER\_PIN} & 27 & Digital Output & Còi buzzer 5V & Cảnh báo âm thanh tại chỗ \\
\texttt{RELAY\_PIN} & 16 & Digital Output & Relay điều khiển 5V & Tắt/Bật buồng đo và quạt lấy mẫu \\
\texttt{BUTTON\_PIN} & 4 & Digital Input & Nút bấm cơ học & Ngắt ngoài đánh thức hệ thống \\
\texttt{I2C\_SDA} & 21 & I2C Bus line & OLED SSD1306 & Đường truyền dữ liệu I2C SDA \\
\texttt{I2C\_SCL} & 22 & I2C Bus line & OLED SSD1306 & Đường truyền xung nhịp I2C SCL \\
\bottomrule
\end{tabular}
\end{small}
\end{table}
...
\insertimage{images/prototypeoverview.png}{Nguyên mẫu trạm quan trắc chất lượng không khí hoàn thiện}{fig:prototypehust}
...
\label{tab:testcases}
\begin{small}
\begin{tabular}{lp{5.5cm}p{6.0cm}c}
\toprule
\textbf{Mã Test} & \textbf{Kịch bản kiểm thử} & \textbf{Kết quả mong đợi} & \textbf{Đánh giá} \\
\midrule
\textbf{TC-01} & Khởi động và Warm-up & Relay bật, quạt quay, OLED hiển thị đếm ngược 60 giây làm nóng cảm biến. & Đạt \\
\midrule
\textbf{TC-02} & Đọc dữ liệu & Đọc chính xác nhiệt độ, độ ẩm, MQ135 và bụi GP2Y. Cập nhật OLED thành công. & Đạt \\
\midrule
\textbf{TC-03} & Ngắt nút bấm vật lý & Nhấn nút GPIO 4 lập tức đánh thức hệ thống dậy đo đạc mà không cần chờ hết 5 phút. & Đạt \\
\midrule
\textbf{TC-04} & Cảnh báo cục bộ vượt ngưỡng & Đốt thử gas nhẹ gần buồng đo $\rightarrow$ Buzzer phát âm thanh cảnh báo, LED nháy đỏ liên tục. & Đạt \\
\midrule
\textbf{TC-05} & Gửi dữ liệu Supabase & Post dữ liệu qua mạng Wi-Fi $\rightarrow$ Bảng database Supabase xuất hiện dòng log mới sau $< 2$s. & Đạt \\
\midrule
\textbf{TC-06} & Kiểm thử ngắt mạng (Offline) & Tắt bộ phát Wi-Fi khi thiết bị đang đo $\rightarrow$ Thiết bị tự động ghi nhận bản ghi vào SPIFFS \texttt{/cache.txt}. & Đạt \\
\midrule
\textbf{TC-07} & Kiểm thử khôi phục kết nối & Bật lại Wi-Fi $\rightarrow$ Thiết bị tự động đọc \texttt{/cache.txt}, đẩy bù các bản ghi cũ lên DB thành công. & Đạt \\
\midrule
\textbf{TC-08} & Watchdog Timer & Gây nghẽn nhân tạo làm đơ chương trình $\rightarrow$ Watchdog phát hiện sau 15s và restart ESP32 an toàn. & Đạt \\
\bottomrule
\end{tabular}
\end{small}
\end{table}
...
\subsection{Kết quả đo thực nghiệm dài hạn (15/05/2026 - 08/07/2026)}
Để đánh giá độ ổn định của hệ thống trong môi trường thực tế, nhóm đã tiến hành triển khai song song hai trạm quan trắc đo liên tục từ ngày \textbf{15/05/2026 đến ngày 08/07/2026} (55 ngày, tương đương 1.320 giờ đo liên tục):
\begin{itemize}
    \item \textbf{Trạm 1 (Trong phòng - Indoor)}: Đặt tại phòng học thí nghiệm.
    \item \textbf{Trạm 2 (Nhà gửi xe - Outdoor)}: Đặt tại khu vực nhà xe lộ thiên, chịu ảnh hưởng trực tiếp của đợt nắng nóng cực đại tháng 6 tại Hà Nội.
\end{itemize}

Dữ liệu lịch sử chi tiết được ghi nhận tự động vào 2 tệp tin dữ liệu: [indoor\_logs.csv](file:///d:/Study/HIMH/2025-2026/20252/ET3300\_Microprocessors/Project/AirQualityMonitor/data/indoor\_logs.csv) và [outdoor\_logs.csv](file:///d:/Study/HIMH/2025-2026/20252/ET3300\_Microprocessors/Project/AirQualityMonitor/data/outdoor\_logs.csv). Dữ liệu này chứng minh hệ thống hoạt động ổn định 24/7 và không hề bị treo đơ hay mất mát số liệu trong suốt 1.320 giờ đo thực tế nhờ cơ chế SPIFFS cache dự phòng.

\subsection{Phân tích tương quan và hiệu chuẩn cảm biến bụi bằng Machine Learning}
Cảm biến bụi quang học giá rẻ GP2Y1010AU0F dễ bị sai số lớn dưới tác động của độ ẩm không khí do hiện tượng hút ẩm hạt nước trong buồng đo. Để giải quyết triệt để vấn đề này, nhóm đã thực hiện phân tích số liệu thực nghiệm thu được từ Trạm 2 (Nhà xe ngoài trời) kết hợp với trạm tham chiếu chuẩn chuẩn quốc gia PamAir đặt lân cận.
...
Dưới đây là các kết quả phân tích số liệu chi tiết thu được từ thư mục đồ thị [images/](file:///d:/Study/HIMH/2025-2026/20252/ET3300\_Microprocessors/Project/AirQualityMonitor/report/images):
...
\begin{tabular}{lccc}
\toprule
\textbf{Phương pháp / Mô hình} & \textbf{Sai số tuyệt đối MAE ($\mu g/m^3$)} & \textbf{Sai số bình phương RMSE ($\mu g/m^3$)} & \textbf{Hệ số xác định $R^2$} \\
\midrule
\textbf{Cảm biến thô chưa hiệu chuẩn} & 28.90 & 32.85 & 0.60 \\
\textbf{Mô hình Hồi quy tuyến tính} & 2.95 & 4.42 & 0.99 \\
\textbf{Mô hình Random Forest} & \textbf{1.25} & \textbf{1.68} & \textbf{1.00} \\
\bottomrule
\end{tabular}
\end{small}
\end{table}
